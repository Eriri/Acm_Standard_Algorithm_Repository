...
struct LCT
{
	...
	void up(int u){sz[u]=sz[ch[u][0]]+sz[ch[u][1]]+1;mx[u]=max(val[u],max(mx[ch[u][0]],mx[ch[u][1]]));}
	void down(int u)
	{
	...
	void make_root(int u){expose(u);splay(u);app_rev(u);}
	int ancestor(int u){while(pre[u])u=pre[u];return u;}
	void link(int c,int f){make_root(c);pre[c]=f;}
	void cut(int f,int c)
	{
		make_root(f);expose(c);splay(c);
		pre[ch[c][0]]=pre[c];pre[c]=0;
		rt[ch[c][0]]=true;ch[c][0]=0;up(c);
	}
	int query(int u,int v){make_root(u);expose(v);splay(v);return mx[v];}
	int lca_query(int a,int b)
	{
		...
		for(a=0;b;a=b,b=pre[b])
		{
			...
			rt[ch[b][1]]=true;rt[a]=false;
			ch[b][1]=a;up(b);
		}
	}
	void change(int a,int b){expose(a);splay(a);val[a]=b;up(a);}
};
